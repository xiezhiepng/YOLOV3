...
def knn_classifier(train_x, train_y):  
    from sklearn.neighbors import KNeighborsClassifier  #使用sklearn作为knn分类器
...
if __name__ == '__main__':
    ...
    saver.restore(sess, model_checkpoint_path)#复原模型
    ...
    data=load_data(data_dir)#标签就是文件名
    keys=[]
    ...
    for x in data[keys[0]]:#对female进行处理
        #检测脸的位置对数据进行编排
        ...
        for face_position in bounding_boxes:
            ...
            crop = cv2.resize(crop, (96, 96), interpolation=cv2.INTER_CUBIC )#resize

            #print(crop.shape)
        
            crop_data=crop.reshape(-1,96,96,3)
            #print(crop_data.shape)
        #完成映射，生成映射，返回128维空间映射
    ...
    for y in data[keys[1]]:#对male进行处理
...
#将测试集和验证集分开
    ...
    classifiers = knn_classifier #knn分类实现预测
